...
public class MarshWorkerWidget implements IWorkerWidget{
	public static final IDimension SIZE = new Dimension(50, 80);

	public static MarshWorkerWidget createFor(Worker worker) {
		return new MarshWorkerWidget(worker);
	}
	...
	private void switchToUnselectedImage() {
		imageLayer.setImage(GameImage.MARSH_WORKER.image);
	}

	private void switchToSelectedImage() {
		imageLayer.setImage(GameImage.MARSH_WORKER_SELECTED.image);
	}

	private void switchToUnavailableImage() {
		imageLayer.setImage(GameImage.MARSH_WORKER_WORKED.image);
	}
